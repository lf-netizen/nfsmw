...
MIN_Y, MIN_X = (569, 128); MIN_R = 16
DIR_NAME = 'minimap_LUT'
RANGE = len(os.listdir(f'data/{DIR_NAME}'))

data  = []
for img_name in sorted(os.listdir(f'data/{DIR_NAME}'), key=lambda name: int(name[:-4])):
    img = cv2.imread(f'data/{DIR_NAME}/{img_name}', cv2.IMREAD_GRAYSCALE)
    data.append(img)
