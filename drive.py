...
from utils import read_angle, angle_diff_norm, preprocess_img, IMG_SIZE_X, IMG_SIZE_Y, read_speed, get_learner
    ...
    while True:
        ...
        if not q_pred.empty():
            ...
            if pred is None:
                no_key()
                break
            accelerate, desired_diff, pred_ad = pred
        ...
        if np.sign(desired_diff)*np.sign(angle_diff) > 0:
            ...
            if np.sign(desired_diff)*pred_ad < 0:
                print('\nchange')
                desired_diff = -np.sign(desired_diff)
...
def predict(learn, q_pred, wincap):
    ...
    while True:
        ...
        q_pred.put((accelerate, angle_diff, pred_ad))
